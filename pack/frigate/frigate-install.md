...
4. 确认系统中存在另一块用于存储监控视频的硬盘。如果不存在需要关机、连接磁盘后再执行后续步骤。

5. 确认系统中是否存在 Coral 硬件设备。如果存在需要首先执行 [coral-install-pcie.sh] 脚本（脚本自动判断操作系统类型并安装驱动）。安装完成后重启服务器再执行后续步骤。
...
6. 执行 [frigate-install.sh] 根据提示进行安装：
...
7. 查看 Frigate 日志，找到 admin 用户的初始 `Password` 并妥善保管。
