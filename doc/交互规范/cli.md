# DEXC Tools｜命令行工具统一交互规范

本规范用于约束 `cli` 目录下所有命令行脚本（macOS `.command`、Windows `.ps1`）的交互风格，确保不同系统下的操作体验一致。

## 核心原则

1. **启动不立即执行**：先展示适用场景、功能说明、操作方式，等待用户输入后再执行。
2. **参数先收齐再执行**：需要多个参数时，逐个输入，全部收集后回显确认，再执行。
3. **路径支持拖拽输入**：允许用户拖动文件/目录到终端，脚本自动规整引号与转义。
4. **分步骤输出进度**：每个步骤结束后输出步骤结果与整体进度。
5. **结束等待确认**：执行结果汇总后，等待用户回车才结束，禁止立即退出。

## 启动说明模板

所有脚本启动后首先输出以下内容（分隔线使用 `=` 或 `-`，避免框线字符错位）：

```
=========================================
  <工具名称>
=========================================
[适用场景]
说明本脚本适用于什么场景。

[功能说明]
说明本脚本执行什么功能、做了什么。

[操作方式]
说明用户如何操作（选择选项、输入路径、拖拽文件等）。

[执行步骤]
1. 步骤一
2. 步骤二
3. 步骤三

[注意事项]
- 破坏性操作警告
- 权限要求
- 其它提示
=========================================
```

## 版本号

- 版本号格式：`主版本.次版本`。
- 位置：启动横幅工具名称正下方一行，形如 `版本 1.0`。
- 变更须递增版本号，并同步更新 README 工具表；版本升级遵循 `主版本`（不兼容/重大变更）与 `次版本`（功能变更/修复）约定。

## 统一信息前缀

所有提示信息统一使用以下前缀，禁止自由发挥：

| 前缀 | 含义 | 用途 |
| ---- | ---- | ---- |
| `[提示]` | 普通提示 | 引导用户操作 |
| `[进度]` | 步骤进度 | 步骤执行中/完成标记 |
| `[完成]` | 成功 | 单步成功 |
| `[失败]` | 失败 | 单步失败 |
| `[错误]` | 输入错误/异常 | 非法输入、运行异常 |
| `[结果]` | 汇总 | 脚本结束前的汇总 |

## 输入约定

### 菜单选择
- 选项使用数字 `1`、`2` ... 编号。
- 提供取消选项：`q`（quit）或 `0`。
- 非法输入**循环重试**，不直接退出。

示例：
```
请输入选项数字 [1 / 2 / q 取消] 然后按回车: 
```

### 多参数输入
- 逐项提示，提示中标注当前进度 `(参数 1/3)`。
- 全部输入完成后**回显所有参数**，用户确认后才执行。
- 允许直接回车使用默认值（如适用），提示中标注 `[回车使用默认值: xxx]`。

### 路径输入与拖拽
- 接受带引号或不带引号的路径输入。
- 接受用户直接拖拽文件/目录到终端。
- 脚本必须对输入做规整后再使用：

**Windows (.ps1)**
- 运行于 PowerShell 控制台，输入路径可能带首尾引号，需去除。
- 目录路径可能在尾部带 `\`，需去除一个尾部反斜杠。
- 规整逻辑：
  ```powershell
  $input = $input.Trim('"')
  $input = $input.TrimEnd('\')
  ```

**macOS (.command)**
- Terminal 拖入路径时空格被转义为 `\ `，需还原；iTerm2 可能带引号。
- 规整逻辑（先去首尾引号，再还原空格转义 `\ `）：
  ```bash
  input=${input%\"}; input=${input#\"}
  input=$(printf '%s' "$input" | sed 's/\\ / /g')
  ```

## 进度与结果输出

### 步骤进度
- 每步执行前输出 `[进度] 步骤 X/N：<步骤名> ...`
- 每步执行后输出 `[完成]` 或 `[失败]`（失败时说明原因）。
- 单步失败不中断流程时，记录失败，最后汇总中列出。

### 结果汇总
脚本结束前统一输出：

```
[结果] 执行完成。
  成功：2/3
  失败：1/3
  - <失败的步骤说明>
```

## 破坏性操作

- 删除文件、修改系统设置、重启进程等操作前必须二次确认。
- 明确提示操作影响范围（例如"执行后访达窗口将全部关闭"）。
- 提供 `q` 取消机会。

## 权限检查

- Windows：需要管理员权限时，先检查并提示，权限不足则警告或退出。
- macOS：需要写系统目录时，检查权限并提示。

## 编码与文件规范

| 文件类型 | 编码 | 换行 | 其它 |
| -------- | ---- | ---- | ---- |
| `.ps1` | UTF-8 无 BOM | LF | Windows 工具，经通用启动器运行（见下文"Windows 启动与运行"） |
| `.command` | UTF-8 无 BOM | LF | 首行 `#!/bin/bash`，需 `chmod +x` |
| `PowerShellRunner.cmd` | 纯 ASCII | CRLF | Windows 通用启动器，内容不得出现非 ASCII 字符 |

- `.command` 禁止添加 BOM，否则会破坏 `#!/bin/bash` 首行导致脚本无法运行。
- Windows `.ps1` 使用 UTF-8 无 BOM；为保证任意环境下正确解码，统一经通用启动器以 UTF-8 读取执行，详见下文。
- 通用启动器（唯一的 `.cmd` 文件）内容必须保持纯 ASCII，并使用 CRLF 换行，避免中文乱码与 `goto`/标签异常。
- 提交/保存时确保编码一致，避免中文乱码。

### 为什么 Windows 不使用 cmd，而改用 PowerShell + 通用启动器

cmd.exe 在 `chcp 65001`（UTF-8 代码页）下解析批处理源码时存在已知缺陷：它按内部字节偏移逐行定位，而 UTF-8 是多字节编码，对中文字符的字节定位会漂移，导致个别行**从错误的字节位置开始读取**。由于 UTF-8 具有自同步性，错位只出现在**行首**（表现为 `'�xx' is not recognized` 式乱码），行内其余文本仍正常。

通过添加 BOM（`EF BB BF`）规避该缺陷也被实测证伪：BOM 会破坏批处理第 1 行（如 `@echo off` 无法执行，回显保持开启），且行首乱码依旧且每次出现的行不固定。因此 Windows 侧不再使用内联中文的 `.cmd`，而是迁移到 PowerShell：

- **`.ps1` 读取差异**：PowerShell 7（`pwsh`）原生按 UTF-8 读取 `.ps1`；Windows PowerShell 5.1 对无 BOM 文件按系统 ANSI 代码页读取（中文 Windows 为 GBK/936），会乱码。
- **特性兜底**：为在"无法预知用户环境"（可能只有自带 5.1）的前提下仍保证 UTF-8 无 BOM 正确显示，统一通过通用启动器运行：启动器用 .NET 以 **UTF-8** 强制读取目标 `.ps1` 内容并执行，不依赖 5.1 的 ANSI 误读，也不要求文件带 BOM。

### Windows 启动与运行

- `.ps1` 默认双击是"编辑打开"而非运行，且受执行策略限制。统一通过通用启动器运行，两种方式均可：
  - **方式一（拖拽）**：把 `.ps1` 文件拖到 `PowerShellRunner.cmd` 上直接执行。
  - **方式二（先启动后拖入）**：双击启动 `PowerShellRunner.cmd`，窗口出现路径输入提示，再把 `.ps1` 拖进该窗口自动填入路径，按回车执行；直接回车则退出。
  - 启动器所有执行路径（成功/失败/取消）末尾都有 `pause`，窗口不会一闪而过。
- **不要**直接右键 `.ps1` → "使用 PowerShell 运行"：该动词调用 `powershell.exe`（5.1），对无 BOM 的 UTF-8 文件按系统 ANSI 代码页读取（中文乱码），且不经过 `-ExecutionPolicy Bypass`（默认 `Restricted` 策略会直接报"禁止运行脚本"并一闪而过）。UTF-8 无 BOM 下唯一可靠路径就是经通用启动器拖拽运行。
- 启动器自动优先调用 `pwsh`，缺失时回落 `powershell`（5.1），并使用 `-NoProfile -ExecutionPolicy Bypass` 绕过执行策略。
- 若日后确实需要右键直跑 `.ps1`：`powershell.exe` 5.1 无法按文件识别 UTF-8 无 BOM，只有两条路——(1) 给 `.ps1` 加 UTF-8 BOM（右键直跑中文即可正常）；(2) 改用 `pwsh`（PowerShell 7 原生按 UTF-8 读取）并把 `.ps1` 的"使用 PowerShell 运行"注册表动词改为指向 `pwsh.exe -File "%1"`。二者都会破坏当前"无 BOM / 仅经通用启动器"的约定，需另行决策，禁止未经批准单方面更改。
- 脚本内建议显式设置控制台编码，保证中文输出正确：
  ```powershell
  $null = chcp 65001
  [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
  ```

## 结束确认

脚本所有输出完成后，等待用户按回车再结束：

- Windows（PowerShell）：脚本以 `Read-Host "按回车键退出..."` 收尾，再 `exit 0`。
- macOS：
  ```bash
  read -p "按回车键退出..."
  ```
