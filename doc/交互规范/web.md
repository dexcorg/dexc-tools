...
- 版本号格式：`主版本.次版本`。
- 位置：页面头部与标题平级放置，外层用 `d-flex align-items-center gap-2` 容器包裹；用 Bootstrap Badge 组件标识（`badge bg-secondary`），并以原生 `<small>` 包裹缩小字号，显示形如 `1.0`，用户可见。
- 版本号不属于文案，不进入 i18n 资源，不随语言切换变化。
