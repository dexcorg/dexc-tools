...
# ==================================================
# Python 通用启动器（macOS · UTF-8 无 BOM）
# 用法一：把 .py 文件拖到此脚本上直接运行
# 用法二：双击启动，自动探测 Python、按需创建 .venv 并安装依赖，
#         随后扫描并列出同目录下所有 .py 脚本，输入编号选择执行，
#         也可直接输入/拖入其它 .py 路径。
# 用法三：PYRunner.command "path/to/script.py" [参数...]
# ==================================================
...
prompt() {
    ...
    done < <(find "$SCRIPT_DIR" -maxdepth 1 -type f -name "*.py" ! -name "PYRunner.command" ! -name "PYRunner.cmd" 2>/dev/null | sort)
